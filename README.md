...
A: 因为每个版本的配置文件都有区别，精力有限，无法处理每一种情况

Q: 为什么反连平台不支持`dns`的方式？

A: 因为绝大多数的反连平台只需要`rmi/http`方式，必须`dns`的数量不多，且配置起来非常麻烦

Q: 为什么点击关闭后要等一段时间才能关？

A: 因为关闭后需要确保所有的`java`进程都退出，否则会出现意外的问题

Q: 为什么我的子域名扫描功能无法运行？

A: 因为你没有使用高级版，这是高级版功能

Q: 为什么我`rad`命令行乱码？

A: 这个问题暂时无法解决，也没必要解决，并不影响功能
